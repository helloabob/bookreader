 ...
 * 													TweenMax.to(mc, 1, {x:300, delay:2, overwrite:1}); //immediately overwrites the previous tween</code>
 * 													
 ...
 * 													TweenMax.to(mc, 1, {x:100, y:200});<br />
 * 													TweenMax.to(mc, 1, {x:300, overwrite:2}); //only overwrites the "x" property in the previous tween</code>
 * 													
 * 									<li><b> 3 (CONCURRENT):</b> Overwrites all tweens of the same object that are active when the tween begins.<br /><code>
 * 													  TweenMax.to(mc, 1, {x:100, y:200});<br />
 * 													  TweenMax.to(mc, 1, {x:300, delay:2, overwrite:3}); //does NOT overwrite the previous tween because the first tween will have finished by the time this one begins.</code>
 * 									</ul>
 * 	
 ...
 */
	public class TweenMax extends TweenLite implements IEventDispatcher {
		/** @private **/
		public static const version:Number = 11.0999;
...
//---- STATIC FUNCTIONS -----------------------------------------------------------------------------------------------------------
		...
		public static function isTweening(target:Object):Boolean {
			...
			var i:int = a.length;
			while (i-- > 0) {
				if ((a[i].active || (a[i].cachedStartTime == a[i].timeline.cachedTime && a[i].timeline.active)) && !a[i].gc) {
					return true;
				}
			}
			return false;
